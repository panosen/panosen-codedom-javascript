    ...
    partial class JsCodeEngine
    {
        private void GenerateIfStepBuilder(IfStepBuilder ifStepBuilder, CodeWriter codeWriter, GenerateOptions options)
        {
            //if
            {
                ...
                GenerateStepBuilderOrCollectionList(ifStepBuilder.StepBuilders, codeWriter, options);
            ...
            //else if
            {
                if (ifStepBuilder.ElseIfStepBuilders != null && ifStepBuilder.ElseIfStepBuilders.Count > 0)
                {
                    foreach (var elseIfStepBuilder in ifStepBuilder.ElseIfStepBuilders)
                    {
                        ...
                        GenerateStepBuilderOrCollectionList(elseIfStepBuilder.StepBuilders, codeWriter, options);
            ...
            //else
            {
                if (ifStepBuilder.ElseStepBuilder != null)
                {
                    ...
                    GenerateStepBuilderOrCollectionList(ifStepBuilder.ElseStepBuilder.StepBuilders, codeWriter, options);
